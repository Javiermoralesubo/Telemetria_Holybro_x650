...
def listar_puertos():
    ...
    # 2. Escanear puertos COM en Windows (COM1 a COM24) para detectar activos adicionales
    if os.name == 'nt':
    ...
    # 3. Añadir opciones fijas de simulación SITL, Telemetría IP y accesos directos COM comunes
    puertos_fijos = [
        ...
        {"id": "COM11", "nombre": "COM11", "tipo": "serial"},
...
# --- PLANIFICADOR DE MISIÓN Y GESTIÓN DE SETPOINTS (QGROUNDCONTROL COMPATIBLE) ---
mision_actual = {
    "puntos": [
        {
            "id": 1,
...
def api_subir_mision_mavlink():
    ...
    with conexion_lock:
        if conexion is None:
            return jsonify({"status": "error", "mensaje": "No hay conexión MAVLink activa con el autopiloto."}), 400
        ...
        with mision_lock:
            ...
            try:
                ...
                for idx, p in enumerate(puntos):
                    ...
                    # Usamos MISSION_ITEM_INT (MAV_FRAME_GLOBAL_RELATIVE_ALT_INT = 3)
